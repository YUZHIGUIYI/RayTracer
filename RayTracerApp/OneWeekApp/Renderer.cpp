...
#include <OneWeek/thread_stream.h>
#include <OneWeek/OneWeekUtils.h>

namespace OneWeek
{
    const vec3 Renderer::background{0.0, 0.0, 0.0};
        ...
        thread_num(1), image_size(width * height), max_depth(50), aspect_ratio(double(width) / double(height)), speed_of_progress(0)
    {
        ...
        double aperture  = 0.0;
        double vfov = 40.0;
#elifdef VOLUMES_RENDER
        // camera - for cornell box smoke volumes
        vec3 lookfrom(278.0, 278.0, -800.0);
        vec3 lookat(278.0, 278.0, 0.0);
        vec3 vup(0.0, 1.0, 0.0);
        double dist_to_focus = 10.0;
        ...
        //world->simple_light();  // two spheres and a rectangle light
#elifdef CORNELLBOX_RENDER
        world->cornell_box();   // cornell box
#elifdef VOLUMES_RENDER
        world->cornell_smoke();   // cornell box with smoke
#else
        world->final_scene();       // final scene
#endif
    ...
    vec3 Renderer::ray_color(const ray& r, int depth)
    {
        ...
        if (world->hit(r, 0.001, infinity, rec))
        {
            ray scattered;
            vec3 attenuation;
            // if the material is light, it will return its color, otherwise it will return vec3(0.0, 0.0, 0.0)
            vec3 emitted = rec.mat_ptr->emitted(rec.u, rec.v, rec.p);
            // once the light material calls "scatter" function, it will return false
            if (!rec.mat_ptr->scatter(r, rec, attenuation, scattered))
                return emitted;
            return emitted + attenuation * ray_color(scattered, depth - 1);
        }

        // hit background - old method
        // vec3 unit_direction = unit_vector(r.direction());
        // auto t = 0.5 * (unit_direction.y() + 1.0);
        // return (1.0 - t) * vec3(1.0, 1.0, 1.0) + t * vec3(0.5, 0.7, 1.0);
        // hit background - new method
        return background;
    }
}
